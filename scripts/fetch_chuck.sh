...
# --- 1b) Daisy MIDI patch ----------------------------------------------------------------------
# Re-introduce ChucK's MidiIn/MidiOut device classes over the Pod UART: strip midiio_rtmidi's RtMidi
...
# docs/dev/chuck-midi-in.md. Pinned to CHUCK_REF; a ref bump may require refreshing the patch.
...
# deliberately NOT compiled (the 166 KB OS backend stays out). See docs/dev/chuck-midi-in.md.
