...
// main loop (the board MIDI poll -> ChuckEngine::handle_midi_message) to the audio ISR, which drains them
// in process() and feeds each into ChucK's re-introduced MidiIn device via MidiInManager::inject() right
// before ck->run(). This carries the whole channel-voice vocabulary - real velocity, NoteOff, CC,
// pitch-bend, aftertouch, program change - plus system realtime (clock/start/stop), so a patch using
// `MidiIn min; min.recv(msg);` sees exactly what a desktop ChucK patch would. (The separate global
// bridge in chuck_engine.cpp still delivers NoteOns as globals for the notesA/noteOnA convention.)
// See docs/dev/chuck-midi-in-porting.md.
...
// message (never blocks the producer). N must be a power of two. Same shape as midi_note.h's NoteQueue.
template <uint32_t N>
class MidiMsgQueue {
