...
// DELIBERATE DEVIATION from the sk-engines original, and the only ported engine that needed one.
// Upstream writes:
//
//     using KitStore = daisy::PersistentStorage<KitData, kKitSlug, 1>;   // three template args
//     g_kit_store.Init(qspi, defaults, kKitQspiOffset);
//     g_kit_store.Save(kd);
//
// That is the bleeptools libDaisy FORK's PersistentStorage, which adds a slug and a version to the
// template and takes the QSPI handle in Init(). Stock libDaisy - what libs/ checks out - has a
// one-argument template, takes the handle in the CONSTRUCTOR, and its Save() takes no argument
// (it writes whatever GetSettings() currently holds). This block is rewritten against the stock API.
//
// What is lost: the fork's slug/version guard, which traps a layout change at boot. Nothing here
// depends on it - KitData carries its own `version` field and apply() rejects a mismatch by falling
// back to defaults, which is the check that actually matters.
//
// The store holds a QSPIHandle reference, so it cannot be constructed before the harness hands us one;
// a function-local static defers construction to the first kit_load() call.
using KitStore = daisy::PersistentStorage<EdrumsEngine::KitData>;
KitStore* g_kit_store = nullptr;

void kit_load(void* qspi_ptr, EdrumsEngine& eng) {
    if (!qspi_ptr) return;
    static KitStore store(*static_cast<daisy::QSPIHandle*>(qspi_ptr));
    g_kit_store = &store;
    EdrumsEngine::KitData defaults;   // FACTORY seed; only applied if a USER save already exists in flash
    store.Init(defaults, kKitQspiOffset);
    if (store.GetState() == KitStore::State::USER)
        eng.apply(store.GetSettings());
}
void kit_save(const EdrumsEngine::KitData& kd) {
    if (!g_kit_store) return;            // no QSPI handle was injected: saving is simply disabled
    g_kit_store->GetSettings() = kd;     // stock Save() persists the live settings struct
    g_kit_store->Save();
}
} // namespace
#endif
