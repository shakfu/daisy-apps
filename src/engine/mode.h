...
// Engine-facing selectors. Contract-owned (item 5b) so the IEngine surface depends on them without
// pulling the granular DSP. `Mode` is granular-specific (the looper's three modes); `Route` is the
// channel topology the platform's mode switch drives. The granular `core/mode.h` redirects here so
// its relative includes still resolve.
...
// The remaining engine-facing enums (item 5b). Contract-owned so the platform code no longer needs to
// include core/modulator.h, core/fx.h, core/deck.h. The granular classes alias these (`using Type = ModType;` etc.), so their
...
// The transport clock source (was Driver::Source), exposed via transport_source().
